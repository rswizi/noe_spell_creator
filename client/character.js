(() => {
      ...
      <td>
        <select class="input" data-sub-skill disabled>
          ...
          ${GROUPS.flatMap(g=>g.skills).map(k=>{
            const name = ({soh:'Sleight of hand', medecine:'Medicine'})[k] || k.charAt(0).toUpperCase()+k.slice(1);
  ...
  const milestoneFromScore  = s   => Math.floor(s/2 - 5);
  const tens                = l   => Math.floor(l/10);
  ...
  function recompute(){
    ...
    // skills (base = invested + Excellence (<= invested) + positive milestone of linked char)
    GROUPS.forEach(g=>{
      const add = Math.max(charMil[g.key]||0, 0);
      g.skills.forEach(sk=>{
        const inv = num($(`[data-skill="${sk}"]`)?.value || 0);
        const ex  = Math.min(inv, exMap[sk] || 0);
        const base= inv + ex + add;
    ...
    const cp_used= GROUPS.reduce((a,g)=> a + num($(`[data-invest="${g.invest}"]`)?.value || 0), 0);
    ...
    const en_max = 5 + Math.floor(lvl_up/5) + (2*mile_wil) + (4*mile_mag) + (2*endur);
    const fo_max = 2 + Math.floor(lvl_up/5) + mile_wil + mile_pre + clarity;
    ...
    // visuals for caps
    GROUPS.forEach(g=>{
      const invInp = $(`[data-invest="${g.invest}"]`);
  ...
  GROUPS.forEach(g=>{
    $(`[data-invest="${g.invest}"]`)?.addEventListener('input', ()=>{ recompute(); debounceSave(); });
  ...
  /* ---------- Save (safe, optional) ---------- */
  let saveTimer = null;
  let currentId = null; // you can set after first POST if your API returns {id}
  ...
  function collectState(){
    const state = {
      ...
      level: Math.max(1, num($('#c_level')?.value||1)),
      xp: num($('#c_xp')?.value||0),
      invested: {},
      skills: {},
    ...
    GROUPS.forEach(g=>{
      state.invested[g.invest] = num($(`[data-invest="${g.invest}"]`)?.value || 0);
      g.skills.forEach(sk=>{
